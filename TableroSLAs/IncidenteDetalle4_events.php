...
function BindEvents()
{
    ...
    $mc_incidentes_t_espera->DHoraEsperaS->CCSEvents["BeforeShow"] = "mc_incidentes_t_espera_DHoraEsperaS_BeforeShow";
...
// -------------------------
	...
	{ 	
		...
		$Container->slAnterior->SetValue($miArray[$claveAnterior]);
   		...
   		$RedirectAnterior =  "IncidenteDetalle4.php?" . CCAddParam( CCRemoveParam( CCGetQueryString("QueryString","Id_incidente"),"ccsForm"),"Id_incidente",$miArray[$claveAnterior]);
	...
	else
	{
   	...
   	$RedirectSiguiente =  "IncidenteDetalle4.php?" . CCAddParam( CCRemoveParam( CCGetQueryString("QueryString","Id_incidente"),"ccsForm"),"Id_incidente",$miArray[$claveSiguiente]);
	...
	if ($TipoCalculoTiempo==1){
		...
		//cambio a partir de que se agregaron las Torres AVL
  		$DBcnDisenio->query("SELECT ClaveMovimiento,DescMovimiento,FechaInicioMov,FechaFinMov,Paquete, dbo.ufDiffFechasMCSec(FechaInicioMov,FechaFinMov) as  HorasInvertidas " .
			...
			" where ( ClaveMovimiento in (select claveMovimiento from mc_c_movimiento where issolucion2=1) ) " .
	...
	if ($TipoCalculoTiempo==2){
		global $DBcnDisenio;
	  	...
	  	$DBcnDisenio->query("select top 1 Paquete, SUM(dbo.ufDiffFechasMCSec(FechaInicioMov,FechaFinMov)) as TotalSegundos from mc_detalle_incidente_avl a " .
							" where Id_Incidente ='" .  CCGetParam("Id_incidente",0)  . "' and ClaveMovimiento in (select claveMovimiento from mc_c_movimiento where issolucion2=1) " . 
...
// -------------------------
	global $Redirec;
   	$Redirec =  "IncidenteDetalle4.php?" . CCAddParam( CCRemoveParam( CCGetQueryString("QueryString","export"),"ccsForm"),"export","1");
...
//mc_incidentes_t_espera_DHoraEsperaS_BeforeShow @467-28F8F914
function mc_incidentes_t_espera_DHoraEsperaS_BeforeShow(& $sender)
{
    $mc_incidentes_t_espera_DHoraEsperaS_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $mc_incidentes_t_espera; //Compatibility
//End mc_incidentes_t_espera_DHoraEsperaS_BeforeShow

//Custom Code @501-2A29BDB7
// -------------------------
	$horas_t_solucion = abs($mc_incidentes_t_espera->horas_t_solucion->GetValue());
	$horas_t_atencion = abs($mc_incidentes_t_espera->horas_t_atencion->GetValue());  

    if ($horas_t_solucion > 0){
    	$mc_incidentes_t_espera->DHoraEsperaS->SetValue(intval($horas_t_solucion));
    	$minutos_dec = ($horas_t_solucion-$mc_incidentes_t_espera->DHoraEsperaS->GetValue() )*60;    	
    	$mc_incidentes_t_espera->DMinEsperaS->SetValue(intval($minutos_dec));
		$segundos_dec = ($minutos_dec-intval($minutos_dec ))*60;
    	$mc_incidentes_t_espera->DSegEsperaS->SetValue(intval($segundos_dec));
    }
    
    if ($mc_incidentes_t_espera->horas_t_solucion->GetValue() > 0){
    	$mc_incidentes_t_espera->operacionTsolucion->SetValue(1); 	
    } else {
    	$mc_incidentes_t_espera->operacionTsolucion->SetValue(-1); 
    }
    
    if ($horas_t_atencion > 0){
    	$mc_incidentes_t_espera->DHoraEsperaA->SetValue(intval($horas_t_atencion));
    	$minutos_decA = ($horas_t_atencion-$mc_incidentes_t_espera->DHoraEsperaA->GetValue() )*60;    	
    	$mc_incidentes_t_espera->DMinEsperaA->SetValue(intval($minutos_decA));
		$segundos_decA = ($minutos_decA-intval($minutos_decA ))*60;
    	$mc_incidentes_t_espera->DSegEsperaA->SetValue(intval($segundos_decA));
    }
    
        if ($mc_incidentes_t_espera->horas_t_atencion->GetValue() > 0){
    	$mc_incidentes_t_espera->operacionTatencion->SetValue(1); 	
    } else {
    	$mc_incidentes_t_espera->operacionTatencion->SetValue(-1); 
    }
// -------------------------
...
//Close mc_incidentes_t_espera_DHoraEsperaS_BeforeShow @467-9403F878
    return $mc_incidentes_t_espera_DHoraEsperaS_BeforeShow;
}
//End Close mc_incidentes_t_espera_DHoraEsperaS_BeforeShow
...
//Page_BeforeInitialize @1-74852A1A
function Page_BeforeInitialize(& $sender)
{
    $Page_BeforeInitialize = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $IncidenteDetalle4; //Compatibility
...
// -------------------------
	...
	//$existe_t_esperaS=0;
	$existe_t_esperaS=CCDLookUp("count(*)","mc_incidentes_t_espera"," id_incidente='".CCGetParam("Id_incidente",0)."' and mes=".CCGetParam("s_mes_param",0)." and anio=".CCGetParam("s_anio_param",0)." and horas_t_solucion is not null", $DBcnDisenio);
	$t_esperaS=$existe_t_esperaS > 0 ? CCDLookUp("horas_t_solucion","mc_incidentes_t_espera","Id_incidente='".CCGetParam("Id_incidente",0)."' and mes=".CCGetParam("s_mes_param",0)." and anio=".CCGetParam("s_anio_param",0), $DBcnDisenio) : 0;
	//$existe_t_esperaS=0;
	$existe_t_esperaA=CCDLookUp("count(*)","mc_incidentes_t_espera"," Id_incidente='".CCGetParam("Id_incidente",0)."' and mes=".CCGetParam("s_mes_param",0)." and anio=".CCGetParam("s_anio_param",0)." and horas_t_atencion is not null", $DBcnDisenio);
	$t_esperaA=$existe_t_esperaA > 0 ? CCDLookUp("horas_t_atencion","mc_incidentes_t_espera"," Id_incidente='".CCGetParam("Id_incidente",0)."' and mes=".CCGetParam("s_mes_param",0)." and anio=".CCGetParam("s_anio_param",0), $DBcnDisenio) : 0;	
...
//Page_OnInitializeView @1-A094652B
...
//Page_AfterInitialize @1-01207CB8
