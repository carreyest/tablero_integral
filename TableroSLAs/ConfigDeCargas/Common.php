...
//Connection Settings @0-F699BE52
...
//Initialize Common Variables @0-B845CAFC
...
//Connection1 Connection Class @-E0F62A73
...
//CCLoginUser @0-DE8A5C42
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT id_acceso, id_grupo, password FROM acceso_conf_carga WHERE login=" . $db->ToSQL($login, ccsText) . " AND password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("id_acceso"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("id_grupo"));
