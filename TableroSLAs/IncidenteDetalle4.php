<?php
//Include Common Files @1-BB2F9575
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "IncidenteDetalle4.php");
...
//Operation Method @143-1687407F
    function Operation()
    {
        ...
        $Redirect = "IncidenteDetalle2.php" . "?" . CCGetQueryString("QueryString", array("ccsForm"));
        if($this->PressedButton == "Cancelar") {
            ...
            if($this->PressedButton == "Button_Insert") {
                $Redirect = "IncidenteDetalle4.php" . "?" . CCGetQueryString("QueryString", array("ccsForm"));
...
class clsRecordmc_incidentes_t_espera { //mc_incidentes_t_espera Class @450-9CB22A4D

//Variables @450-9E315808
...
//Class_Initialize Event @450-80202D5F
    function clsRecordmc_incidentes_t_espera($RelativePath, & $Parent)
    {
        ...
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Record mc_incidentes_t_espera/Error";
        $this->DataSource = new clsmc_incidentes_t_esperaDataSource($this);
        $this->ds = & $this->DataSource;
        $this->InsertAllowed = true;
        $this->UpdateAllowed = true;
        $this->DeleteAllowed = true;
        ...
        if($this->Visible)
        {
            $this->ComponentName = "mc_incidentes_t_espera";
            $this->Attributes = new clsAttributes($this->ComponentName . ":");
            $CCSForm = explode(":", CCGetFromGet("ccsForm", ""), 2);
            if(sizeof($CCSForm) == 1)
                $CCSForm[1] = "";
            ...
            $this->H_mes = new clsControl(ccsHidden, "H_mes", "H_mes", ccsText, "", CCGetRequestParam("H_mes", $Method, NULL), $this);
            $this->H_anio = new clsControl(ccsHidden, "H_anio", "H_anio", ccsText, "", CCGetRequestParam("H_anio", $Method, NULL), $this);
            $this->DHoraEsperaS = new clsControl(ccsTextBox, "DHoraEsperaS", "DHoraEsperaS", ccsInteger, "", CCGetRequestParam("DHoraEsperaS", $Method, NULL), $this);
            $this->DMinEsperaS = new clsControl(ccsTextBox, "DMinEsperaS", "DMinEsperaS", ccsInteger, "", CCGetRequestParam("DMinEsperaS", $Method, NULL), $this);
            $this->DSegEsperaS = new clsControl(ccsTextBox, "DSegEsperaS", "DSegEsperaS", ccsInteger, "", CCGetRequestParam("DSegEsperaS", $Method, NULL), $this);
            $this->horas_t_solucion = new clsControl(ccsHidden, "horas_t_solucion", "Horas T Solucion", ccsFloat, "", CCGetRequestParam("horas_t_solucion", $Method, NULL), $this);
            $this->DHoraEsperaA = new clsControl(ccsTextBox, "DHoraEsperaA", "DHoraEsperaA", ccsInteger, "", CCGetRequestParam("DHoraEsperaA", $Method, NULL), $this);
            $this->DMinEsperaA = new clsControl(ccsTextBox, "DMinEsperaA", "DMinEsperaA", ccsInteger, "", CCGetRequestParam("DMinEsperaA", $Method, NULL), $this);
            $this->DSegEsperaA = new clsControl(ccsTextBox, "DSegEsperaA", "DSegEsperaA", ccsInteger, "", CCGetRequestParam("DSegEsperaA", $Method, NULL), $this);
            $this->horas_t_atencion = new clsControl(ccsHidden, "horas_t_atencion", "Horas T Atencion", ccsFloat, "", CCGetRequestParam("horas_t_atencion", $Method, NULL), $this);
            $this->operacionTsolucion = new clsControl(ccsListBox, "operacionTsolucion", "Operacion_T_Solucion", ccsText, "", CCGetRequestParam("operacionTsolucion", $Method, NULL), $this);
            $this->operacionTsolucion->DSType = dsListOfValues;
            $this->operacionTsolucion->Values = array(array("1", "Sumar"), array("-1", "Restar"));
            $this->operacionTsolucion->Required = true;
            $this->operacionTatencion = new clsControl(ccsListBox, "operacionTatencion", "Operacion_T_Atencion", ccsText, "", CCGetRequestParam("operacionTatencion", $Method, NULL), $this);
            ...
            if(!$this->FormSubmitted) {
                ...
                if(!is_array($this->H_anio->Value) && !strlen($this->H_anio->Value) && $this->H_anio->Value !== false)
                    $this->H_anio->SetText(CCGetParam("s_anio_param"));
                if(!is_array($this->operacionTsolucion->Value) && !strlen($this->operacionTsolucion->Value) && $this->operacionTsolucion->Value !== false)
                    $this->operacionTsolucion->SetText(1);
                if(!is_array($this->operacionTatencion->Value) && !strlen($this->operacionTatencion->Value) && $this->operacionTatencion->Value !== false)
                    $this->operacionTatencion->SetText(1);
            }
            if(!is_array($this->mes->Value) && !strlen($this->mes->Value) && $this->mes->Value !== false)
                $this->mes->SetText(CCGetParam("s_mes_param"));
            if(!is_array($this->anio->Value) && !strlen($this->anio->Value) && $this->anio->Value !== false)
...
//Initialize Method @450-3E703885
    function Initialize()
    {
...
//Validate Method @450-5AB687EE
    function Validate()
    {
        ...
        $Validation = ($this->H_mes->Validate() && $Validation);
        $Validation = ($this->H_anio->Validate() && $Validation);
        $Validation = ($this->DHoraEsperaS->Validate() && $Validation);
        $Validation = ($this->DMinEsperaS->Validate() && $Validation);
        $Validation = ($this->DSegEsperaS->Validate() && $Validation);
        $Validation = ($this->horas_t_solucion->Validate() && $Validation);
        $Validation = ($this->DHoraEsperaA->Validate() && $Validation);
        $Validation = ($this->DMinEsperaA->Validate() && $Validation);
        $Validation = ($this->DSegEsperaA->Validate() && $Validation);
        $Validation = ($this->horas_t_atencion->Validate() && $Validation);
        $Validation = ($this->operacionTsolucion->Validate() && $Validation);
        $Validation = ($this->operacionTatencion->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->H_id_incidente->Errors->Count() == 0);
        $Validation =  $Validation && ($this->H_mes->Errors->Count() == 0);
        $Validation =  $Validation && ($this->H_anio->Errors->Count() == 0);
        $Validation =  $Validation && ($this->DHoraEsperaS->Errors->Count() == 0);
        $Validation =  $Validation && ($this->DMinEsperaS->Errors->Count() == 0);
        $Validation =  $Validation && ($this->DSegEsperaS->Errors->Count() == 0);
        $Validation =  $Validation && ($this->horas_t_solucion->Errors->Count() == 0);
        $Validation =  $Validation && ($this->DHoraEsperaA->Errors->Count() == 0);
        $Validation =  $Validation && ($this->DMinEsperaA->Errors->Count() == 0);
        $Validation =  $Validation && ($this->DSegEsperaA->Errors->Count() == 0);
        $Validation =  $Validation && ($this->horas_t_atencion->Errors->Count() == 0);
        $Validation =  $Validation && ($this->operacionTsolucion->Errors->Count() == 0);
        $Validation =  $Validation && ($this->operacionTatencion->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @450-FE5242A8
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->mes->Errors->Count());
        ...
        $errors = ($errors || $this->H_mes->Errors->Count());
        $errors = ($errors || $this->H_anio->Errors->Count());
        $errors = ($errors || $this->DHoraEsperaS->Errors->Count());
        $errors = ($errors || $this->DMinEsperaS->Errors->Count());
        $errors = ($errors || $this->DSegEsperaS->Errors->Count());
        $errors = ($errors || $this->horas_t_solucion->Errors->Count());
        $errors = ($errors || $this->DHoraEsperaA->Errors->Count());
        $errors = ($errors || $this->DMinEsperaA->Errors->Count());
        $errors = ($errors || $this->DSegEsperaA->Errors->Count());
        $errors = ($errors || $this->horas_t_atencion->Errors->Count());
        $errors = ($errors || $this->operacionTsolucion->Errors->Count());
        $errors = ($errors || $this->operacionTatencion->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
        $errors = ($errors || $this->DataSource->Errors->Count());
        return $errors;
    }
...
//InsertRow Method @450-794CA776
    function InsertRow()
    {
        ...
        $this->DataSource->H_mes->SetValue($this->H_mes->GetValue(true));
        $this->DataSource->H_anio->SetValue($this->H_anio->GetValue(true));
        $this->DataSource->DHoraEsperaS->SetValue($this->DHoraEsperaS->GetValue(true));
        $this->DataSource->DMinEsperaS->SetValue($this->DMinEsperaS->GetValue(true));
        $this->DataSource->DSegEsperaS->SetValue($this->DSegEsperaS->GetValue(true));
        $this->DataSource->horas_t_solucion->SetValue($this->horas_t_solucion->GetValue(true));
        $this->DataSource->DHoraEsperaA->SetValue($this->DHoraEsperaA->GetValue(true));
        $this->DataSource->DMinEsperaA->SetValue($this->DMinEsperaA->GetValue(true));
        $this->DataSource->DSegEsperaA->SetValue($this->DSegEsperaA->GetValue(true));
        $this->DataSource->horas_t_atencion->SetValue($this->horas_t_atencion->GetValue(true));
        $this->DataSource->operacionTsolucion->SetValue($this->operacionTsolucion->GetValue(true));
        $this->DataSource->operacionTatencion->SetValue($this->operacionTatencion->GetValue(true));
        $this->DataSource->Insert();
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterInsert", $this);
        return (!$this->CheckErrors());
    }
...
//Show Method @450-DFBE781D
    function Show()
    {
        ...
        $this->operacionTsolucion->Prepare();
        $this->operacionTatencion->Prepare();
        ...
        if($this->EditMode) {
            ...
            if($this->DataSource->Errors->Count() == 0 && $this->DataSource->next_record()) {
                ...
                if(!$this->FormSubmitted){
                    ...
                    $this->H_mes->SetValue($this->DataSource->H_mes->GetValue());
                    $this->H_anio->SetValue($this->DataSource->H_anio->GetValue());
                    $this->horas_t_solucion->SetValue($this->DataSource->horas_t_solucion->GetValue());
                    $this->horas_t_atencion->SetValue($this->DataSource->horas_t_atencion->GetValue());
                }
            } else {
                $this->EditMode = false;
            }
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->H_mes->Errors->ToString());
            $Error = ComposeStrings($Error, $this->H_anio->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DHoraEsperaS->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DMinEsperaS->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DSegEsperaS->Errors->ToString());
            $Error = ComposeStrings($Error, $this->horas_t_solucion->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DHoraEsperaA->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DMinEsperaA->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DSegEsperaA->Errors->ToString());
            $Error = ComposeStrings($Error, $this->horas_t_atencion->Errors->ToString());
            $Error = ComposeStrings($Error, $this->operacionTsolucion->Errors->ToString());
            $Error = ComposeStrings($Error, $this->operacionTatencion->Errors->ToString());
            $Error = ComposeStrings($Error, $this->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DataSource->Errors->ToString());
            $Tpl->SetVar("Error", $Error);
            $Tpl->Parse("Error", false);
        ...
        $this->H_mes->Show();
        $this->H_anio->Show();
        $this->DHoraEsperaS->Show();
        $this->DMinEsperaS->Show();
        $this->DSegEsperaS->Show();
        $this->horas_t_solucion->Show();
        $this->DHoraEsperaA->Show();
        $this->DMinEsperaA->Show();
        $this->DSegEsperaA->Show();
...
} //End mc_incidentes_t_espera Class @450-FCB6E20C

class clsmc_incidentes_t_esperaDataSource extends clsDBcnDisenio {  //mc_incidentes_t_esperaDataSource Class @450-332DC539

//DataSource Variables @450-7D60481E
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
    ...
    public $DMinEsperaS;
    public $DSegEsperaS;
    public $horas_t_solucion;
    public $DHoraEsperaA;
    public $DMinEsperaA;
    public $DSegEsperaA;
    public $horas_t_atencion;
    public $operacionTsolucion;
    public $operacionTatencion;
//End DataSource Variables

//DataSourceClass_Initialize Event @450-5DE4DFAE
    function clsmc_incidentes_t_esperaDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Record mc_incidentes_t_espera/Error";
        $this->Initialize();
        ...
        $this->DHoraEsperaS = new clsField("DHoraEsperaS", ccsInteger, "");
        
        $this->DMinEsperaS = new clsField("DMinEsperaS", ccsInteger, "");
        
        $this->DSegEsperaS = new clsField("DSegEsperaS", ccsInteger, "");
        
        $this->horas_t_solucion = new clsField("horas_t_solucion", ccsFloat, array(True, 3, Null, "", False, array("#"), array("0", "0", "0"), 1, True, ""));
        
        $this->DHoraEsperaA = new clsField("DHoraEsperaA", ccsInteger, "");
        
        $this->DMinEsperaA = new clsField("DMinEsperaA", ccsInteger, "");
        
        $this->DSegEsperaA = new clsField("DSegEsperaA", ccsInteger, "");
        
        $this->horas_t_atencion = new clsField("horas_t_atencion", ccsFloat, array(True, 3, Null, "", False, array("#"), array("0", "0", "0"), 1, True, ""));
        
        $this->operacionTsolucion = new clsField("operacionTsolucion", ccsText, "");
        
        $this->operacionTatencion = new clsField("operacionTatencion", ccsText, "");
        

        $this->InsertFields["id_incidente"] = array("Name" => "id_incidente", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->InsertFields["mes"] = array("Name" => "mes", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->InsertFields["anio"] = array("Name" => "anio", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->InsertFields["horas_t_solucion"] = array("Name" => "horas_t_solucion", "Value" => "", "DataType" => ccsFloat, "OmitIfEmpty" => 1);
        $this->InsertFields["horas_t_atencion"] = array("Name" => "horas_t_atencion", "Value" => "", "DataType" => ccsFloat, "OmitIfEmpty" => 1);
        $this->UpdateFields["id_incidente"] = array("Name" => "id_incidente", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["mes"] = array("Name" => "mes", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["anio"] = array("Name" => "anio", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        $this->UpdateFields["horas_t_solucion"] = array("Name" => "horas_t_solucion", "Value" => "", "DataType" => ccsFloat, "OmitIfEmpty" => 1);
        $this->UpdateFields["horas_t_atencion"] = array("Name" => "horas_t_atencion", "Value" => "", "DataType" => ccsFloat, "OmitIfEmpty" => 1);
    }
//End DataSourceClass_Initialize Event

//Prepare Method @450-D654A9F8
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
...
//Open Method @450-74AF7058
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM mc_incidentes_t_espera {SQL_Where} {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query(CCBuildSQL($this->SQL, $this->Where, $this->Order));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
    }
...
//SetValues Method @450-28749B9A
    function SetValues()
    {
        ...
        $this->H_mes->SetDBValue($this->f("mes"));
        $this->H_anio->SetDBValue($this->f("anio"));
        $this->horas_t_solucion->SetDBValue(trim($this->f("horas_t_solucion")));
        $this->horas_t_atencion->SetDBValue(trim($this->f("horas_t_atencion")));
    }
//End SetValues Method

//Insert Method @450-A593563B
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->InsertFields["mes"]["Value"] = $this->H_mes->GetDBValue(true);
        $this->InsertFields["anio"]["Value"] = $this->H_anio->GetDBValue(true);
        $this->InsertFields["horas_t_solucion"]["Value"] = $this->horas_t_solucion->GetDBValue(true);
        $this->InsertFields["horas_t_atencion"]["Value"] = $this->horas_t_atencion->GetDBValue(true);
        $this->SQL = CCBuildInsert("mc_incidentes_t_espera", $this->InsertFields, $this);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteInsert", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteInsert", $this->Parent);
...
//Update Method @450-82B485A3
    function Update()
    {
        ...
        $this->UpdateFields["mes"]["Value"] = $this->H_mes->GetDBValue(true);
        $this->UpdateFields["anio"]["Value"] = $this->H_anio->GetDBValue(true);
        $this->UpdateFields["horas_t_solucion"]["Value"] = $this->horas_t_solucion->GetDBValue(true);
        $this->UpdateFields["horas_t_atencion"]["Value"] = $this->horas_t_atencion->GetDBValue(true);
        $this->SQL = CCBuildUpdate("mc_incidentes_t_espera", $this->UpdateFields, $this);
        $this->SQL .= strlen($this->Where) ? " WHERE " . $this->Where : $this->Where;
        if (!strlen($this->Where) && $this->Errors->Count() == 0) 
            $this->Errors->addError($CCSLocales->GetText("CCS_CustomOperationError_MissingParameters"));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteUpdate", $this->Parent);
...
//Delete Method @450-4B2F1466
    function Delete()
    {
        ...
        $Where = "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildDelete", $this->Parent);
        $this->SQL = "DELETE FROM mc_incidentes_t_espera";
        $this->SQL = CCBuildSQL($this->SQL, $this->Where, "");
        if (!strlen($this->Where) && $this->Errors->Count() == 0) 
            $this->Errors->addError($CCSLocales->GetText("CCS_CustomOperationError_MissingParameters"));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteDelete", $this->Parent);
...
} //End mc_incidentes_t_esperaDataSource Class @450-FCB6E20C
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "IncidenteDetalle4.html";
$BlockToParse = "main";
...
//Include events file @1-57E2D63F
include_once("./IncidenteDetalle4_events.php");
...
$mc_incidentes_t_espera = new clsRecordmc_incidentes_t_espera("", $MainPage);
...
$MainPage->mc_incidentes_t_espera = & $mc_incidentes_t_espera;
...
$mc_incidentes_t_espera->Initialize();
$ScriptIncludes = "";
...
//Execute Components @1-A07B28CC
$mc_incidentes_t_espera->Operation();
...
//Go to destination page @1-5F013F48
if($Redirect)
{
    ...
    unset($mc_incidentes_t_espera);
    unset($Tpl);
    exit;
}
//End Go to destination page

//Show Page @1-F546F926
