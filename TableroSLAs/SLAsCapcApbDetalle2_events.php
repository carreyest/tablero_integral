...
function BindEvents()
{
    ...
    $mc_info_rs_ap_EC->DHoraSAT->CCSEvents["BeforeShow"] = "mc_info_rs_ap_EC_DHoraSAT_BeforeShow";
...
//mc_info_rs_ap_EC_DHoraSAT_BeforeShow @213-84DB25F0
function mc_info_rs_ap_EC_DHoraSAT_BeforeShow(& $sender)
{
    $mc_info_rs_ap_EC_DHoraSAT_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $mc_info_rs_ap_EC; //Compatibility
//End mc_info_rs_ap_EC_DHoraSAT_BeforeShow

//Custom Code @220-2A29BDB7
// -------------------------
 
     if ($mc_info_rs_ap_EC->HorasSAT->GetValue() > 0){
    	$mc_info_rs_ap_EC->DHoraSAT->SetValue(intval($mc_info_rs_ap_EC->HorasSAT->GetValue()));
    	$minutos_dec = ($mc_info_rs_ap_EC->HorasSAT->GetValue()-$mc_info_rs_ap_EC->DHoraSAT->GetValue() )*60;    	
    	$mc_info_rs_ap_EC->DMinSAT->SetValue(intval($minutos_dec));
		$segundos_dec = ($minutos_dec-intval($minutos_dec ))*60;
    	$mc_info_rs_ap_EC->DSegSAT->SetValue(intval($segundos_dec));
    }

// -------------------------
//End Custom Code

//Close mc_info_rs_ap_EC_DHoraSAT_BeforeShow @213-A8EA61CA
    return $mc_info_rs_ap_EC_DHoraSAT_BeforeShow;
}
//End Close mc_info_rs_ap_EC_DHoraSAT_BeforeShow
...
// -------------------------
	...
	$sSQL= 'SELECT e.estimacion_id, sum(CAST(e.esfuerzo AS FLOAT)) esfuerzo, e.tipo_unidad, sum(cast(replace(e.unidades_resultantes,\',\',\'.\') AS FLOAT)) unidades_resultantes, ' .
			' e.requerimiento_relacionado, e.solicitante, max(e.fecha_asignacion) fecha_asignacion, e.ID_PPMC, e.Proveedor, max(e.FECHA_APROBACION) FECHA_APROBACION, ' .
			' max(e.ESTADO_REQ_ESTIM) estado_req_estim' .
			' FROM ppmc_estimacion e ' .
			' WHERE ID_PPMC= ' . CCGetParam("s_numero",0) . ' and PROVEEDOR =\'Itera, S.A. de C.V.\' and month(e.fecha_carga)=' .  CCGetParam("s_mes",0) . 'and YEAR(e.FECHA_CARGA) =' .  CCGetParam("s_anio",0) .
				' and e.ESTADO_REQ_ESTIM = \'Estimación Aprobada\' and e.RESULTADO_ESTIMACION <> \'Rechazada\'' . 
				' group by e.estimacion_id, e.tipo_unidad, unidades_resultantes,   e.requerimiento_relacionado, e.solicitante, e.ID_PPMC, e.Proveedor';
				//echo($sSQL);
...
// -------------------------
    ...
    if($db->next_record()){ 
    	if($db->f(0)==0){ // si no existe se inserta
			$sUpdate="insert into mc_calificacion_capc  (id_ppmc, id_proveedor, id_servicio_negoico, id_serviciocont, [descripción], mesreporte, anioreporte, HERR_EST_COST, REQ_SERV,  obs_manuales, id, id_tipo, SLO) " .
...
//Page_BeforeShow @1-9C6FA8EB
function Page_BeforeShow(& $sender)
{
    $Page_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $SLAsCapcApbDetalle2; //Compatibility
//End Page_BeforeShow
...
// -------------------------
    ...
    if(count($aPPMCsAPbIds)>1){
    	...
    	if($iPos==0){
			...
			$QueryString = CCAddParam($QueryString, "sID", $aPPMCsAPbIds[$iPos-1]);
			$QueryString = CCAddParam($QueryString, "s_numero", $aPPMCsAPbValues[$iPos-1]);
			$QueryString = CCRemoveParam($QueryString, "ccsForm");
    		$lkAnterior->SetLink("SLAsCapcApbDetalle2.php?" . $QueryString);

    	}
    	if($iPos < count($aPPMCsAPbIds)-1){
    		$lkSiguiente->SetValue($aPPMCsAPbValues[$iPos+1]);
    		//$lkSiguiente->SetLink("SLAsCapcApbDetalle.php?" . CCAddParam( CCRemoveParam( CCGetQueryString("QueryString","sID"),"ccsForm"),"sID",$aPPMCsAPbIds[$iPos+1]));
    		$QueryString = CCGetQueryString("QueryString", "");
			$QueryString = CCAddParam($QueryString, "sID", $aPPMCsAPbIds[$iPos+1]);
			$QueryString = CCAddParam($QueryString, "s_numero", $aPPMCsAPbValues[$iPos+1]);
			$QueryString = CCRemoveParam($QueryString, "ccsForm");
    		$lkSiguiente->SetLink("SLAsCapcApbDetalle2.php?" . $QueryString);
	...
	switch ($tipoquery){
		case "ListItems":
			...
			$post_string='<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">'.
			  ...
			  '				 <And><Contains><FieldRef Name="FileRef" /><Value Type="Text">Proyecto</Value></Contains>'.
