...
//Page_BeforeShow @1-14104FDC
function Page_BeforeShow(& $sender)
{
    $Page_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $TableroSLAs_Car; //Compatibility
//End Page_BeforeShow
...
// -------------------------
	...
	//******************
	$idprov= CCGetParam('s_id_proveedor',0);
	$mes= CCGetParam('s_MesReporte',0);
	$anio=CCGetParam('s_AnioReporte',2);
	$tipo_sla_slo=CCGetParam('sSLO',0);
	if ($tipo_sla_slo==1) 
		$tipo_sla_slo="SLO";  
	else 
		$tipo_sla_slo="SLA"; 
		
	$cadPeriodo=" select distinct m.id_periodo from  archivosxls.dbo.l_calificacion_rs_aut m, 	archivosxls.dbo.periodos_carga p
		where m.id_periodo=p.id_periodo and anio=$anio and mes=$mes 
	and m.id_proveedor =$idprov
	and m.tipo_nivel_servicio ='".$tipo_sla_slo."'
	and m.estatus ='F' ";
	...
	if(CCGetParam('s_id_proveedor',0)==0){
		...
		if(CCDLookUp("UrlTableroCDS","mc_c_proveedor","id_proveedor = " . $db->ToSQL(CCGetParam('s_id_proveedor',0),ccsInteger),$db)==""){
			...
			//ojo quitar la sig linea
			$UrlCDS->SetValue("Comparativo de Mediciones");
			
			$UrlCDS->SetLink("http://webiterasrv2:8080/MyMSDMA4/comparativo.php?s_id_periodo=$s_id_periodo&s_id_proveedor=$idprov&s_opt_slas=$tipo_sla_slo");
		} else {
...
//Page_OnInitializeView @1-55313E86
...
//Page_AfterInitialize @1-6F0559DE
