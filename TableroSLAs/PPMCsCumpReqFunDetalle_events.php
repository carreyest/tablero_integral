...
//Page_BeforeShow @1-BF44FFAC
function Page_BeforeShow(& $sender)
{
    $Page_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $PPMCsCumpReqFunDetalle; //Compatibility
//End Page_BeforeShow
...
// -------------------------
	...
	if(count($aPPMCsAPbIds)>1){
    	...
    	if($iPos==0){
    		...
    		$lkAnterior->SetLink("PPMCsCumpReqFunDetalle.php?" . CCAddParam( CCRemoveParam( CCGetQueryString("QueryString","Id"),"ccsForm"),"Id",$aPPMCsAPbIds[$iPos-1]));
    	}
    	if($iPos < count($aPPMCsAPbIds)-1){
    		$lkSiguiente->SetValue($aPPMCsAPbValues[$iPos+1]);
    		$lkSiguiente->SetLink("PPMCsCumpReqFunDetalle.php?" . CCAddParam( CCRemoveParam( CCGetQueryString("QueryString","Id"),"ccsForm"),"Id",$aPPMCsAPbIds[$iPos+1]));
    	} else {
