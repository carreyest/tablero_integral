...
//Custom Code @69-2A29BDB7
	...
	*/
	...
	$consTableroCAPC="select  sc.descripcion, 
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='herr_est_cost') as meta_herr_est_cost,   
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='req_serv') as meta_req_serv,	 
   ...
   sum(case when cumpl_req_func='1' or cumpl_req_func='0' then 1 else 0 end) totcumpl_req_func, 
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='cumpl_req_func') as meta_cumpl_req_func,
   ...
   sum(case when calidad_prod_term='1' or calidad_prod_term='0' then 1 else 0 end) totcalidad_prod_term, 
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='calidad_prod_term') as meta_calidad_prod_term,
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='retr_entregable') as meta_retr_entregable,
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='cal_cod') as meta_cal_cod,
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='def_fug_amb_prod') as meta_def_fug_amb_prod,
...
count(cumple_inc_tiempoasignacion) totinc_tiempoasignacion, 
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='inc_tiempoasignacion') as meta_inc_tiempoasignacion,
	 ...
	 (select meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='inc_tiemposolucion') as meta_inc_tiemposolucion,
   
	 	 AVG(Cumple_EF) cumplen_efic_presup, AVG(total_ef) tot_efic_presup, avg(cast(Cumple_EF as float))/avg(total_ef)*100  efic_presup,
	 (Select Meta from [TableroMyM_SDMA4_Local].[dbo].mc_c_metrica where acronimo='EFIC_PRESUP') as meta_efic_presup	 
from [TableroMyM_SDMA4_Local].[dbo].mc_c_ServContractual sc 
left join (  select distinct m.*  FROM [TableroMyM_SDMA4_Local].[dbo].[mc_calificacion_rs_MC] m,
   [TableroMyM_SDMA4_Local].[dbo].[mc_universo_cds] u
   where u.numero=m.id_ppmc and SLO=$n_opt_slas and m.AnioReporte=u.anio and m.MesReporte=u.mes
   and m.AnioReporte=$anio_reporte and m.MesReporte=$mes_reporte  and m.id_proveedor = $s_id_proveedor
	 ...
	 left join	(select Cumple_Inc_TiempoAsignacion, Cumple_Inc_TiempoSolucion, 
				...
				from [TableroMyM_SDMA4_Local].[dbo].mc_calificacion_incidentes_MC
	...
	left join (select SUM(case when EficienciaPresupuestal='1' or EficienciaPresupuestal='0' then 1 else 0 end) Cumple_EF, COUNT(EficienciaPresupuestal) Total_EF, Id_Proveedor,  2 IdServicioCont  
			from [TableroMyM_SDMA4_Local].[dbo].mc_eficiencia_presupuestal 
